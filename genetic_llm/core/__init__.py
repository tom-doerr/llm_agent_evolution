...
class Agent:
    def __init__(self, chromosomes: tuple[Chromosome, ...], fitness: float = 0.0):
        ...
        if len({c.type for c in chromosomes}) != len(chromosomes):
            raise ValueError(f"Duplicate chromosome types in {chromosomes}")
            
        if not 0.0 <= fitness <= 1.0:
            raise ValueError(f"Invalid fitness {fitness:.2f} - must be 0.0-1.0")
